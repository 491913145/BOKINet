...
class SGD(object):
    def __init__(self, parameters, lr=0.01, beta=0.01):
    ...
    def update(self):
        for p in self.parameters:
            p.data += self.lr * (p.grad + p.grad_p * (1 - self.beta))
            p.grad_p = p.grad


class Adam(object):
    def __init__(self, parameters, lr=0.01, decay=0, beta1=0.9, beta2=0.999, eps=1e-8):
        self.beta1 = beta1
        self.beta2 = beta2
